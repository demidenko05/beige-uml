...
public class SaxDiagramPackageFiller<DRI, SD extends ISettingsDraw, IMG, DLI> extends ASaxDiagramUmlInteractiveFiller<AsmDiagramPackage<DRI, SD, IMG, FileAndWriter, DLI>, DiagramUml, DRI, SD, IMG, DLI> {
  ...
  public SaxDiagramPackageFiller(String namePersistable, IFactoryAsmElementUml<IAsmElementUmlInteractive<ClassFull<ClassUml>, DRI, SD, FileAndWriter>, DRI, SD, FileAndWriter, ClassFull<ClassUml>> factoryAsmClassFull,
      ...
      IFactoryAsmElementUml<IAsmElementUmlInteractive<ShapeFullVarious<PackageUml>, DRI, SD, FileAndWriter>, DRI, SD, FileAndWriter, ShapeFullVarious<PackageUml>> factoryAsmPackageFull,
    ...
    this.factoryAsmPackageFull = factoryAsmPackageFull;
    ...
    saxPackageFiller = new SaxPackageFiller<PackageUml>(SrvSaveXmlPackage.NAMEXML_PACKAGEUML, getPathCurrent());
  ...
  public boolean fillModel(String elementName, String elementValue) {
    ...
    else if(saxPackageFiller.getModel() != null) {
      if(saxPackageFiller.fillModel(elementName, elementValue)) {
        return true;
      }
    }
  ...
  public boolean fillModel(String elementName, String attrName, String attrValue) {
    ...
    if(super.fillModel(elementName, attrName, attrValue)) {
      ...
      if(saxPackageFiller.fillModel(elementName, attrName, attrValue)) {
        return true;
      }
    }
    else if(saxRelationshipBinaryVariousFiller.getModel() != null) {
  ...
  public boolean handleStartElement(String localName) {
    ...
    if(saxPackageFiller.getNamePersistable().equals(localName)) {
      IAsmElementUmlInteractive<ShapeFullVarious<PackageUml>, DRI, SD, FileAndWriter> asmPackageFull = factoryAsmPackageFull.createAsmElementUml();
      getModel().getAsmListAsmPackagesFull().addElementUml(asmPackageFull);
      asmPackageFull.getElementUml().setIsNew(false);
      saxPackageFiller.setModelAndPrepare(asmPackageFull.getElementUml().getShape());
      return true;
    }
    ...
    if(saxPackageFiller.getModel() != null && saxPackageFiller.handleStartElement(localName)) {
      return true;
    }
  ...
  public boolean handleEndElement(String localName) throws Exception {
    ...
    if(saxRelationshipSelfFiller.getNamePersistable().equals(localName)) {
      ...
      UtilsRectangleRelationship.evalPointsJointAndShared(saxRelationshipSelfFiller.getModel());
      saxRelationshipSelfFiller.setModelAndPrepare(null);
      return true;
    }
    if(saxPackageFiller.getNamePersistable().equals(localName)) {
      saxPackageFiller.setModelAndPrepare(null);
      return true;
    }
    if(saxRelationshipBinaryVariousFiller.getNamePersistable().equals(localName)) {
    ...
    if(saxPackageFiller.getModel() != null && saxPackageFiller.handleEndElement(localName)) {
      return true;
    }
